...
describe('SBT API routes', () => {
  ...
  describe('POST /api/sbt/create-token', () => {
    it('returns success response', async () => {
      ...
      const res = await request(app)
        ...
        .attach('image', Buffer.from('fake'), { filename: '1.png', contentType: 'image/png' });
          ...
          items: [
            {
              ...
              communityTokenAddress: '0x70997970C51812dc3A010C7d01b50e0d17dc79C8',
              nftAddress: null,
              environment: 'dev',
            },
          ],
    ...
    it('returns token list', async () => {
      vi.mocked(sbtService.listTokens).mockResolvedValue({
        ...
        result: {
          ...
          items: [{ dbId: 1, tokenId: '1', name: 'Badge', description: null, image: null, votingPower: '100', isRevoked: false }],
        },
      });
  ...
  describe('POST /api/sbt/batch-mint', () => {
    ...
    it('returns NOT_DAO_MANAGER', async () => {
      vi.mocked(sbtService.batchMint).mockResolvedValue({ ok: false, reason: 'NOT_DAO_MANAGER' });
      const res = await request(app)
        .post('/api/sbt/batch-mint')
        .send({
          environment: 'dev',
    ...
    it('returns success response', async () => {
      vi.mocked(sbtService.uploadMetadata).mockResolvedValue({
        ...
        result: {
          ...
          metadata: { name: 'STG', description: 'STG', attributes: [], image: '' },
        },
      });
  ...
  describe('POST /api/sbt/scheduled-issuances/:id/cancel', () => {
    it('cancels a pending issuance', async () => {
      ...
      const res = await request(app).post('/api/sbt/scheduled-issuances/7/cancel');

      expect(res.status).toBe(200);
      expect(res.body.status).toBe('SUCCESSFUL_CANCEL_SCHEDULED_ISSUANCE');
      expect(vi.mocked(sbtService.cancelScheduledIssuance)).toHaveBeenCalledWith({ id: 7 });
    });
    ...
    it('returns 404 when issuance does not exist', async () => {
      ...
      const res = await request(app).post('/api/sbt/scheduled-issuances/999/cancel');
