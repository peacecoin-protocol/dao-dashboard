...
vi.mock('../../src/lib/pinataAPI.js', () => ({
  ...
  bufferToFile: vi.fn((buffer, name, type) => new File([buffer], name, { type })),
  createMetadataFile: vi.fn((metadata, imageCid) =>
    new File([JSON.stringify(metadata)], `${imageCid}.json`, { type: 'application/json' })
  ),
  buildIpfsUrl: vi.fn((cid: string) => `https://ipfs-dao-studio.mypinata.cloud/ipfs/${cid}`),
...
describe('SbtService', () => {
  ...
  describe('batchMint', () => {
    ...
    it('returns NOT_DAO_MANAGER when signer lacks role', async () => {
      vi.mocked(blockchainService.hasPermissionForBatchMint).mockResolvedValue(false);
    ...
    it('batch mints selected tokens', async () => {
      vi.mocked(blockchainService.hasPermissionForBatchMint).mockResolvedValue(true);
    ...
    it('returns FAILED_TO_MINT on chain error', async () => {
      vi.mocked(blockchainService.hasPermissionForBatchMint).mockResolvedValue(true);
      vi.mocked(blockchainService.batchMint).mockRejectedValue(new Error('revert'));

      const result = await service.batchMint({ ...params, tokens: [params.tokens[0]] });
  ...
  describe('listContracts', () => {
    it('returns contract options from Supabase', async () => {
      ...
      const result = await service.listContracts({ environment: 'dev', assetType: 'sbt' });
      expect(result.ok).toBe(true);
      if (result.ok) {
  ...
  describe('createSbt', () => {
    ...
    it('uploads metadata, creates token on-chain, and saves to Supabase', async () => {
      ...
      vi.mocked(blockchainService.hasPermissionForCreateToken).mockResolvedValue(true);
      vi.mocked(addFilesToGroupPublic)
        .mockResolvedValueOnce({ cid: 'image1' } as Awaited<ReturnType<typeof addFilesToGroupPublic>>)
        .mockResolvedValueOnce({ cid: 'meta1' } as Awaited<ReturnType<typeof addFilesToGroupPublic>>);
  ...
  describe('uploadMetadata', () => {
    ...
    it('uploads image then metadata and returns tokenUri', async () => {
      vi.mocked(addFilesToGroupPublic)
        .mockResolvedValueOnce({ cid: 'imageCid' } as Awaited<ReturnType<typeof addFilesToGroupPublic>>)
        .mockResolvedValueOnce({ cid: 'metadataCid' } as Awaited<ReturnType<typeof addFilesToGroupPublic>>);
  ...
  describe('setTokenUri', () => {
    ...
    it('returns NOT_DAO_MANAGER when signer lacks role', async () => {
      vi.mocked(blockchainService.hasPermissionForSetTokenUri).mockResolvedValue(false);
  ...
  describe('listCommunityTokens', () => {
    it('returns community tokens with creation counts', async () => {
      vi.mocked(supabaseService.listCommunityTokensWithCreations).mockResolvedValue([
        {
          daoId: dao.daoId,
          daoName: dao.daoName,
          communityTokenAddress: dao.tokenAddress,
