...
type ResolvedDao = { ok: true; dao: DaoRecord } | { ok: false; reason: 'DAO_NOT_FOUND' };
...
export class SbtService {
  private async resolveDao(environment: Environment, daoId: string): Promise<ResolvedDao> {
  ...
  private contractAddressForAsset(dao: DaoRecord, assetType: AssetType): string | null {
  ...
  async listContracts(params: { environment: Environment; assetType: AssetType }) {
    try {
  ...
  async createSbt(params: {
    ...
    try {
      const resolved = await this.resolveDao(params.environment, params.daoId);

      if (!resolved.ok) {
        return resolved;
      }

      const sbtAddress = this.contractAddressForAsset(resolved.dao, params.assetType);
      if (!sbtAddress) {
        return { ok: false as const, reason: 'SBT_CONTRACT_NOT_FOUND' as const };
      }
      ...
      try {
        const creator = await blockchainService.getSignerAddress(params.environment);
        const dbToken = await supabaseService.insertToken({
          daoId: resolved.dao.daoId,
  ...
  async listTokens(params: {
    ...
    try {
      ...
      const contractAddress = this.contractAddressForAsset(resolved.dao, params.assetType);
      if (!contractAddress) {
        return { ok: false as const, reason: 'SBT_CONTRACT_NOT_FOUND' as const };
      }
      ...
      return {
        ...
        result: {
          ...
          dao: {
            ...
            tokenAddress: resolved.dao.tokenAddress,
            nftAddress: resolved.dao.nftAddress,
          },
          items,
        },
      };
  ...
  async scheduleIssuance(params: {
    ...
    // Fail early if the signer cannot mint; roles rarely change between
    // scheduling and execution, and this gives immediate feedback in the UI.
    const canMint = await blockchainService.hasPermissionForBatchMint(
      params.sbtAddress,
      params.environment
    );
    ...
    try {
      const record = await supabaseService.insertScheduledIssuance(params);
  ...
  async cancelScheduledIssuance(params: { id: number }) {
    try {
      ...
      const existing = await supabaseService.findScheduledIssuanceById(params.id);
      if (!existing) {
        return { ok: false as const, reason: 'SCHEDULED_ISSUANCE_NOT_FOUND' as const };
      }
      return {
        ok: false as const,
  ...
  async uploadMetadata(params: {
    ...
    try {
      ...
      return { ok: false as const, reason: 'FAILED_TO_UPLOAD_IMAGE' as const, error: errorMessage(err) };
    }
    ...
    try {
      ...
      const jsonUploadResult = await addFilesToGroupPublic(jsonFile, jsonGroupId);
      ...
      return { ok: false as const, reason: 'FAILED_TO_UPLOAD_METADATA' as const, error: errorMessage(err) };
    }
  ...
  private async applyTokenUri(params: {
    ...
    try {
      ...
      return { ok: false as const, reason: 'FAILED_TO_SET_TOKEN_URI' as const, error: errorMessage(err) };
    }
  }

  async listCommunityTokens(params: { environment: Environment }) {
    try {
      const items = await supabaseService.listCommunityTokensWithCreations(params.environment);
      return {
        ok: true as const,
        result: {
          environment: params.environment,
  ...
  async getWalletBalances(params: {
    ...
    try {
      ...
      const dao = resolved.dao;
      const contractAddress = this.contractAddressForAsset(dao, params.assetType);
      if (!contractAddress) {
        return { ok: false as const, reason: 'SBT_CONTRACT_NOT_FOUND' as const };
      }
      ...
      return { ok: false as const, reason: 'FAILED_TO_FETCH_BALANCE' as const, error: errorMessage(err) };
    }
  }
}
