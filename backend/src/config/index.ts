...
function resolveEnv(primary: string, ...fallbackKeys: string[]): string | undefined {
...
export const config = {
  ...
  daoManagerPrivateKey: () => requireEnv('DAO_MANAGER_PRIVATE_KEY'),
  supabaseUrl: () =>
  ...
  issuanceSchedulerEnabled: optionalEnv('ISSUANCE_SCHEDULER_ENABLED', 'true') !== 'false',
  ...
  validateForStartup(): string[] {
    ...
    if (isPlaceholder(process.env.DAO_MANAGER_PRIVATE_KEY)) {
      warnings.push('DAO_MANAGER_PRIVATE_KEY is not set — batch-mint and set-token-uri will fail.');
    }
