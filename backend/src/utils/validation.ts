...
const addressSchema = z
  .string()
  .refine((val) => ethers.isAddress(val), { message: 'Invalid Ethereum address' })
...
export const batchMintSchema = z.object({
  ...
  to: addressSchema,
  tokens: z.array(batchMintTokenSchema).min(1, 'At least one token is required'),
});
...
export const scheduleIssuanceSchema = z.object({
  ...
  executeAt: z
    .string()
    .datetime({ offset: true, message: 'executeAt must be an ISO 8601 datetime' })
