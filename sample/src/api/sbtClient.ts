...
const SIGNER_PRIVATE_KEY_HEADER = 'x-signer-private-key'
...
function signerHeaders(
  ...
  headers.set(SIGNER_PRIVATE_KEY_HEADER, privateKey)
  return headers
}
...
export async function createSbt(
  ...
  const res = await fetch(`${API_BASE}/api/sbt/create-token`, {
    method: 'POST',
    headers: signerHeaders(request.signerPrivateKey),
    body: form,
  })
...
export async function batchMint(
  ...
  const { signerPrivateKey, ...body } = request
  const res = await fetch(`${API_BASE}/api/sbt/batch-mint`, {
    method: 'POST',
    headers: signerHeaders(signerPrivateKey, {
...
export async function cancelScheduledIssuance(
  ...
  const res = await fetch(
    `${API_BASE}/api/sbt/scheduled-issuances/${id}/cancel`,
    {
      method: 'POST',
    }
  )
