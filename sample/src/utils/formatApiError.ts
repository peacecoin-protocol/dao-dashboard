...
const STATUS_HINTS: Record<string, string> = {
  FAILED_TO_CREATE_TOKEN:
    'On-chain token creation failed. The backend wallet must be default admin on the contract.',
  ...
  FAILED_TO_LIST_COMMUNITY_TOKENS: 'Could not load community tokens from Supabase for this environment.',
  NOT_DAO_MANAGER:
    'The backend wallet needs minter role to mint and default admin role to create tokens. Check DAO_MANAGER_PRIVATE_KEY permissions on the contract.',
  FAILED_TO_SET_TOKEN_URI:
    'Token ID must already exist on the contract. Use Create SBT to register new tokens.',
  FAILED_TO_MINT:
    'Minting failed. Token IDs must exist on the contract and the backend wallet must be a minter.',
  FAILED_TO_UPLOAD_IMAGE: 'Pinata image upload failed. Check PINATA_JWT and group IDs in backend/.env.',
  FAILED_TO_UPLOAD_METADATA: 'Pinata metadata upload failed. Check PINATA_JWT and JSON group ID.',
  DAO_NOT_FOUND: 'No DAO in Supabase matches this daoId and environment.',
  SBT_CONTRACT_NOT_FOUND: 'The DAO record is missing a valid contract address for the selected asset type.',
  FAILED_TO_FETCH_BALANCE: 'Could not read balances from Supabase or Polygon RPC.',
  ...
  FAILED_TO_LIST_SCHEDULED_ISSUANCES: 'Could not load scheduled issuances from Supabase.',
  FAILED_TO_CANCEL_SCHEDULED_ISSUANCE: 'Could not cancel the scheduled issuance.',
...
const REVERT_HINTS: Array<{ pattern: RegExp; hint: string }> = [
  {
    ...
    hint: 'The backend wallet does not have default admin role on this contract.',
  },
  {
  ...
  },
  {
    ...
    hint: 'The backend wallet is not registered as a minter on this contract.',
  },
];
...
export function formatApiError(response: ApiResponseLike): FormattedError | null {
