...
function item(partial: Partial<WalletBalanceItem> & Pick<WalletBalanceItem, 'tokenId'>): WalletBalanceItem {
  return {
...
describe('resolveTokenImageUrl', () => {
  ...
  it('returns http urls unchanged', () => {
    expect(resolveTokenImageUrl('https://example.com/a.png')).toBe('https://example.com/a.png');
  });
  ...
  it('resolves bare cids via gateway', () => {
    expect(resolveTokenImageUrl('bafy123')).toBe('https://ipfs-dao-studio.mypinata.cloud/ipfs/bafy123');
  });
});
...
describe('sortMintableTokens', () => {
  it('sorts by token id then db id', () => {
    const tokens: MintableTokenItem[] = [
      { dbId: 2, tokenId: '4', name: 'B', description: null, image: null, votingPower: null, isRevoked: false },
      { dbId: 1, tokenId: '4', name: 'A', description: null, image: null, votingPower: null, isRevoked: false },
      { dbId: 3, tokenId: '2', name: 'C', description: null, image: null, votingPower: null, isRevoked: false },
    ];
