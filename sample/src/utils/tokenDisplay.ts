...
const PINATA_GATEWAY =
  import.meta.env.VITE_PINATA_GATEWAY_URL ?? 'https://ipfs-dao-studio.mypinata.cloud';
...
export function parseEtherToWei(ether: string): string | null {
  ...
  try {
    const wei = BigInt(wholePart) * WEI_PER_ETHER + BigInt(fractionPart.padEnd(18, '0'));
    return wei.toString();
  } catch {
    return null;
  }
...
export function formatVotingPowerToEther(wei: string | null | undefined): string {
...
export function resolveTokenImageUrl(image: string | null | undefined): string | null {
  if (!image?.trim()) {
    return null;
  }
...
export function sortItemsByTokenId(items: WalletBalanceItem[]): WalletBalanceItem[] {
  return [...items].sort((a, b) => compareTokenIds(a.tokenId, b.tokenId));
}

export function sortMintableTokens(items: MintableTokenItem[]): MintableTokenItem[] {
